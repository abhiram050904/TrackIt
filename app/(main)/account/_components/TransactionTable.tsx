...
import { categoryColors } from "@/data/categeries";
import { bulkDeleteTransactions } from "@/actions/TransactionActions";
...
export function TransactionTable({
  ...
  const handleDelete = async (id: string) => {
    ...
    setDeleteLoading(true); // Start loading state
    setError(null); // Reset error state

    try {
      await bulkDeleteTransactions([id]); // Perform the deletion
      toast.success("Transaction deleted successfully");
    } catch (error: any) {
      setError(error.message); // Set error message
      toast.error(error.message); // Show error toast
    } finally {
      setDeleteLoading(false); // End loading state
    }
  };
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      await bulkDeleteTransactions(selectedIds); // Perform the deletion
              ...
              <TableRow key={transaction.id}>
                ...
                <TableCell className="capitalize">
                  <span
                    style={{
                      background:
                        categoryColors[transaction.category] || "#D1D5DB", // Default color if not found
                    }}
